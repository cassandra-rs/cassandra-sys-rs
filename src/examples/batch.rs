...
fn insert_into_batch_with_prepared(session: &mut CassSession, prepared: &CassPrepared, pairs: Vec<Pair>)
    ...
    unsafe {
        ...
        for pair in pairs {
            ...
            cass_statement_bind_string(statement, 1, str2ref(value));
            cass_batch_add_statement(batch, statement);
            cass_statement_free(statement);
        }

        let statement1 = "INSERT INTO examples.pairs (key, value) VALUES ('c', '3')";
        let statement1 = cass_statement_new(str2ref(statement1), 0);
        ...
        let statement2 = cass_statement_new(str2ref(statement2), 2);

        let key = "d";
        let value = "4";

        cass_statement_bind_string(statement2, 0, str2ref(key));
        cass_statement_bind_string(statement2, 1, str2ref(value));
